...
public class DragResizeMod {
    ...
    private static final OnDragResizeEventListener defaultListener = new OnDragResizeEventListener() {
        ...
        public void onResize(Node node, double x, double y, double h, double w) {
            ...
            if (y < 0) y = 0;
            */
            System.out.println("In onResize");
            setNodeSize(node, x, y, h, w);
        }
        ...
        private void setNodeSize(Node node, double x, double y, double h, double w) {
            ...
            // so you have to write resize code anyway for your Nodes...
            //something like this
            if (node instanceof Canvas) {
                ...
                node.setStyle("-fx-height:" + h + ";-fx-width:" + w + ";");                     //.setWidth(w);                
            }
    ...
    public DragResizeMod(Node node, OnDragResizeEventListener listener) {
        this.node = node;
        if (listener != null)
    ...
    public static void makeResizable(Node node, OnDragResizeEventListener listener) {
        ...
        node.setOnMousePressed(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                System.out.println("In OnMousePressed");
        ...
        node.setOnMouseDragged(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                System.out.println("In OnMouseDragged");
        ...
        node.setOnMouseMoved(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                System.out.println("In OnMouseMoved");
        ...
        node.setOnMouseReleased(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                System.out.println("In OnMouseReleased");
    ...
    private S currentMouseState(MouseEvent event) {
        System.out.println("currentMouseState");
        S state = S.DEFAULT;
    ...
    private static Cursor getCursorForState(S state) {
        System.out.println("getCursorForState");
        switch (state) {
            case NW_RESIZE:
    ...
    private void setNewInitialEventCoordinates(MouseEvent event) {
        System.out.println("setNewInitialEventCoordinates");
        nodeX = nodeX();
        nodeY = nodeY();
        nodeH = nodeH();
        nodeW = nodeW();
    ...
    private boolean isInResizeZone(MouseEvent event) {
        System.out.println("isInResizeZone");
    ...
    private boolean isInDragZone(MouseEvent event) {
        System.out.println("isInDragZone");
